...
def build_prompt(user_name, self_ratings, test_scores, materials):
    ...
    low_self_count = sum(1 for r in self_ratings if r < 7)
    low_test_count = sum(1 for s in test_scores if s < 75)
...
4. Для каждой области в формате:

**{номер}. {название области}**

📊 **Результаты:** самооценка — {балл}/10, тест — {процент}%
...
6. В конце добавь блоки:

💡 **Совет:** Регулярно возвращайтесь к этим материалам и отслеживайте прогресс. Маленькие шаги каждый день приводят к большим результатам!

📌 **Что делать дальше?**
Обсудите результаты с руководителем и сформируйте индивидуальный план развития (ИПР).
