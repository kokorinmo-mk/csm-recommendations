...
def build_prompt(user_name, self_ratings, test_scores, materials):
    """Формирует промпт с принудительным выбором ТОП-3 низких областей"""
    ...
    # Предполагаем, что self_ratings и test_scores идут в том же порядке, что и materials
    area_scores = []
    for i, (area, items) in enumerate(materials.items()):
        if i < len(self_ratings) and i < len(test_scores):
            ...
            # Вычисляем проблемный балл (чем ниже, тем хуже)
    ...
    # Сортируем по проблемному баллу (от большего к меньшему)
    area_scores.sort(key=lambda x: x['problem_score'], reverse=True)
    
    # Берём ТОП-3
    top_areas = area_scores[:3]
    
    # Формируем список областей для промпта
    top_areas_text = ""
    for a in top_areas:
...
3. Для КАЖДОЙ области напиши в формате:
...
💡 **Совет:** Регулярно возвращайтесь к этим материалам и отслеживайте свой прогресс. Маленькие шаги каждый день приводят к большим результатам!
