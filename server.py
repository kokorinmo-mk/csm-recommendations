...
def build_prompt(user_name, self_ratings, test_scores, materials):
    """Формирует промпт — GigaChat сам выбирает области, НО с проверкой максимальных результатов"""
...
2. ИНАЧЕ: определи области, которые требуют развития (где самооценка < 7 ИЛИ тест < 75%).
   ...
   - Если таких областей больше 3: выбери ТОП-3 с самыми низкими баллами.
...
3. НЕ ДОБАВЛЯЙ никаких советов, заключений или блоков "что делать дальше" — только то, что указано в правилах.
